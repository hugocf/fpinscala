package fpinscala.ch15streamingio

import fpinscala.ch13iomonad.{IO,Monad,Free,unsafePerformIO}
...
object SimpleStreamTransducers {
  ...
  object Process {
    ...
    import fpinscala.ch13iomonad.Monad
...
object GeneralizedStreamTransducers {
  ...
  object Process {
    ...
    import fpinscala.ch13iomonad.IO
    ...
    def runLog[O](src: Process[IO,O]): IO[IndexedSeq[O]] = IO {
      ...
      def go(cur: Process[IO,O], acc: IndexedSeq[O]): IndexedSeq[O] =
        cur match {
          ...
          case Await(req,recv) =>
            val next =
              try recv(Right(fpinscala.ch13iomonad.unsafePerformIO(req)(E)))
    ...
    import fpinscala.ch13iomonad.IO0.fahrenheitToCelsius
